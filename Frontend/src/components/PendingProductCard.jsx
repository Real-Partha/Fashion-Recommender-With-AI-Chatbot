...
const PendingProductCard = ({ order }) => {
    ...
    const handleApproveOrder = async () => {
        ...
        const response = await fetch("http://127.0.0.1:8000/orders/admin/approve/", {
            method: "POST",
            headers: {
  ...
  return (
    <div className="pending-product-card">
      ...
      <p>Date: {order.date}</p>
      <p>Time: {order.time}</p>
      <button className="pending-product-card-approve" onClick={handleApproveOrder}>Approve Order</button>
    </div>
  );
};
